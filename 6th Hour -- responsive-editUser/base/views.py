...
def loginPage(request):
    ...
    if request.method == "POST":
        ...
        if user is not None:
            ...
            print("=== usernaem or password doen't exist")
    context={"page":page}
...
def home(request):
    ...
    rooms = Room.objects.filter(
        Q(topic__name__icontains=q) | # we use Q to use the logic | and & 
        Q(name__icontains=q) |
        Q(description__icontains=q)
        ) #__icontains to get word start with the searchiing letters (insensetive)
...
def room(request, pk):
    ...
    if request.method == 'POST':
        ...
        room.participants.add(request.user) #add the user to be participant in the room
...
def updateRoom(request, pk):
    ...
    context = {'form':form, "topics":topics, "room":room, "update":update}
    return render(request, 'base/room_form.html', context)
